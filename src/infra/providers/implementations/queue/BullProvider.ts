...
import { IMailQueueProvider } from "@infra/providers/models/IMailQueueProviders";
...
export class BullProvider implements IMailQueueProvider {
  ...
  process(processFunction: Processor<IDeliverMessageJob>): void {
    new Worker("surtur.collection-queue", processFunction, {
      connection: redisConnection,
      concurrency: 100,
      limiter: {
        max: 400,
    ...
    new QueueScheduler("surtur.collection-queue", {
      connection: redisConnection,
    });
  }
}
